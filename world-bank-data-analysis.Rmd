...
#  datatable(df, style = 'bootstrap4', rownames = FALSE, filter = 'top',
#            options = list(scrollX = TRUE)) %>%
#    formatRound(names(dplyr::select_if(df, is.numeric)), round_digits)
#}
...
world_gold_cor <- world_joined_df %>% 
  ...
  filter(abs(`Gold price (USD)`) > 0.6, abs(`Gold price (USD)`) < 0.9) %>%
