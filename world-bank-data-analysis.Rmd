...
# TODO Describe DF is not correct and currency exchange rates are unambiguous
```
...
infmort_plot <- wdi_continents_df %>%
  ...
  rename(`GDP per capita (USD)` = gdp_per_capita_current_us,
         `Mortality rate, infant (per 1,000 live births)` = mortality_rate_infant_per_1_000_live_births,
         Continent = continent,
...
world_joined_df <- wdi_cleaned_df %>%
  ...
  #inner_join(yearly_btc_prices_df, by = c('Year' = 'year')) %>%
